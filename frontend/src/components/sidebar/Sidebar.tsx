...
export function Sidebar({
      ...
      <TooltipTrigger asChild>
        <Button
          ...
          className={
            tool === targetTool
              ? "bg-accent relative"
              : "text-foreground /60 relative"
          }
        >
          <Icon className="w-5 h-5" />
          <p className="absolute text-[9px] bottom-0.5 right-0.5 text-muted-foreground">
            {TOOL_HOTKEYS[targetTool]}
          </p>
        </Button>
      </TooltipTrigger>
  ...
  const [activeColorMode, setActiveColorMode] = useState<"border" | "fill">(
    "border",
  );
      ...
      icon: (
        ...
        >
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"
          />
        </svg>
      ),
    },
    ...
    <>
      <div className="hidden md:flex fixed left-4 top-1/2 -translate-y-1/2 items-center z-50 pointer-events-none select-none">
        <aside className="relative z-20 flex flex-col items-center gap-y-1 p-2 bg-background border shadow-xl rounded-2xl pointer-events-auto">
          ...
          <ToolButton targetTool="magic" icon={Wand2} />
            ...
            <div className="flex gap-2">
              {STROKE_WIDTHS.map((width) => (
                <Button
                  key={width.id}
                  variant={"outline"}
                  className={`w-8 h-8 ${options.strokeWidth == width.value ? "dark:bg-accent bg-accent" : ""}`}
                  onClick={() => {
                    setOptions({ ...options, strokeWidth: width.value });
                  }}
                >
                  {width.label}
                </Button>
              ...
              {STROKE_TYPES.map((type) => (
                <Button
                  ...
                  onClick={() => {
                    setOptions({
                      ...options,
                      strokeType: type.id as ToolOptions["strokeType"],
                    });
                  }}
          ...
          <div className="flex flex-col gap-2">
                ...
                return (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border border-black/5 ${
                      activeColorValue === c
                        ? "scale-110 ring-blue-500/50 ring-2 ring-offset-1"
                        : ""
                    }`}
                    title={c}
                  />
            ...
            <div className="flex justify-between items-center gap-2 mt-1 mr-2">
              ...
              <Input
                ...
                value={
                  activeColorValue === "transparent" ? "" : activeColorValue
                }
